...
VERSION = 'T0162_B9_MARKET_COMPARE_BOARD_V0_1_HARDENED'
OUTPUT_BASENAME = 'B9_MARKET_COMPARE_BOARD_V0'
CANONICAL_CSV_BASENAME = 'B9_MARKET_COMPARE_BOARD'
EXCLUDED_SCAN_DIRS = {'.git', '__pycache__', '_extract', '.pytest_cache', '.mypy_cache'}
...
def is_scan_excluded(path: Path) -> bool:
    ...
    return any(name.lower() in parts for name in EXCLUDED_SCAN_DIRS)


def score_candidate_path(path: Path) -> Tuple[int, float, str]:
    """Prefer real runtime outputs and active docs over temp/extract copies."""
    ...
    if '/samples/' in normalized:
        score -= 20
    if '/downloads/' in normalized or '/_extract/' in normalized:
        score -= 30
...
def recursive_find_by_basename(core_root: Path, basenames: Sequence[str]) -> Optional[Path]:
    ...
    if not candidates:
        return None
    candidates.sort(key=score_candidate_path, reverse=True)
    return candidates[0]
...
def find_input_runtime(core_root: Path, key: str) -> Tuple[Optional[Path], InputRecord]:
    ...
    candidates = INPUT_CANDIDATES.get(key, [])
    for rel in candidates:
        path = core_root / rel
        if path.exists():
            return path, InputRecord(key, required, True, str(path), 'runtime', sha256_file(path), note='exact_candidate')
    fallback = recursive_find_by_basename(core_root, [Path(rel).name for rel in candidates])
    if fallback and fallback.exists():
        return fallback, InputRecord(key, required, True, str(fallback), 'runtime', sha256_file(fallback), note='recursive_fallback')
...
def build_source_quality_summary(scene: Dict[str, Any], records: List[InputRecord], matches: List[MatchRow]) -> Dict[str, Any]:
    text = ' '.join([stringify(scene), ' '.join(stringify(asdict(m)) for m in matches)]).upper()
    found = [r.input_name for r in records if r.found]
    missing_required = [r.input_name for r in records if r.required and not r.found]
    missing_optional = [r.input_name for r in records if (not r.required) and (not r.found)]
    flags = []
    for marker in ['PROXY', 'RAW', 'PARTIAL', 'STALE', 'DEGRADED', 'UNKNOWN', 'MISSING']:
        if marker in text:
            flags.append(marker)
    if missing_required:
        verdict = 'COMPARISON_BLOCKED_REQUIRED_INPUT_MISSING'
    elif 'PROXY' in flags or 'PARTIAL' in flags or 'DEGRADED' in flags or 'STALE' in flags:
        verdict = 'COMPARISON_ALLOWED_SOURCE_QUALITY_LIMITED'
    else:
        verdict = 'COMPARISON_ALLOWED_SOURCE_QUALITY_READABLE'
    return {
        'verdict': verdict,
        'found_inputs': found,
        'missing_required_inputs': missing_required,
        'missing_optional_inputs': missing_optional,
        'detected_quality_markers': flags,
        'doctrine_boundary': 'source_quality_qualifie_la_comparaison_sans_transformer_en_prediction',
    }
...
def build_board(mode: str, core_root: Path, sample_dir: Path, output_dir: Path, top_k: int) -> Dict[str, Any]:
    ...
    board = {
        ...
        'current_scene': scene,
        'source_quality_summary': build_source_quality_summary(scene, records, matches),
        'compare_summary': {
    ...
    # Canonical names requested by T0162 mission.
    write_csv(output_dir / f'{CANONICAL_CSV_BASENAME}_MATCHES_V0.csv', [asdict(m) for m in matches])
    write_csv(output_dir / f'{CANONICAL_CSV_BASENAME}_DIFFERENCES_V0.csv', [asdict(d) for d in differences])
    write_csv(output_dir / f'{CANONICAL_CSV_BASENAME}_TECHNICAL_RISKS_V0.csv', [asdict(r) for r in risks])
    write_csv(output_dir / f'{CANONICAL_CSV_BASENAME}_INPUTS_V0.csv', [asdict(r) for r in records])
    # Backward-compatible aliases for the initial V0 package.
    ...
    manifest = {
        ...
        'outputs': [
            ...
            f'{CANONICAL_CSV_BASENAME}_MATCHES_V0.csv',
            f'{CANONICAL_CSV_BASENAME}_DIFFERENCES_V0.csv',
            f'{CANONICAL_CSV_BASENAME}_TECHNICAL_RISKS_V0.csv',
            f'{CANONICAL_CSV_BASENAME}_INPUTS_V0.csv',
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser.add_argument('--strict-exit', action='store_true', help='Return non-zero when board_state is not PASS. Default keeps runtime orchestration non-breaking.')
    ...
    return 0 if (board['board_state'] == 'PASS' or not args.strict_exit) else 2
