...
START = "# --- T0109_B9_RETEST_SOURCE_SIGNALS_V0_START ---"
END = "# --- T0109_B9_RETEST_SOURCE_SIGNALS_V0_END ---"
...
def patch_forward_tests(repo_root: Path) -> None:
    replacements = {
        repo_root / "tests" / "test_t0107_b9_natural_flow_reading_v0.py": [
            (
                'assert payload["raw_calibration"]["version"] in {"T0107_NATURAL_FLOW_READING_V0", "T0108_RETEST_MIXED_SPLIT_V0"}',
...
def main() -> int:
    ...
    if missing:
        raise RuntimeError("T0109 requires T0108 + T0108A first. Missing: " + ", ".join(missing))

    block = append_file.read_text(encoding="utf-8").strip() + "\n"
    target.write_text(text.rstrip() + "\n\n" + block, encoding="utf-8")
    print("[OK] appended T0109 retest source signals V0 block")
