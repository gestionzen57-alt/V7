...
START = "# --- T0110A_B9_RETEST_SOURCE_FIELDS_METADATA_COMPAT_HOTFIX_START ---"
END = "# --- T0110A_B9_RETEST_SOURCE_FIELDS_METADATA_COMPAT_HOTFIX_END ---"
...
def main() -> int:
    ...
    if missing:
        raise RuntimeError(
            "T0110A requires T0110/T0109/T0108/T0107 first. Missing: " + ", ".join(missing)
        )

    block = append_file.read_text(encoding="utf-8").strip() + "\n"
    target.write_text(text.rstrip() + "\n\n" + block, encoding="utf-8")
    print("[OK] appended T0110A metadata compatibility hotfix block")
    return 0
