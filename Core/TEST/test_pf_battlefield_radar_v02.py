...
from pf_battlefield_radar_v02 import (
...
def main() -> None:
    payload = {
        ...
        "strong_coalition_windows": [
            ...
            {
                "time_key": "2026-05-01T23:12:00+00:00",
                "strong_coalitions": [
                    {
                        "members": ["CHF", "EUR"],
                        "state": "HIGH_PRESSURE_COALITION_FOLDING",
    ...
    scenes = build_battlefield_scenes_from_scan_payload(payload, dedupe=True)
    assert len(scenes) == 2
    assert scenes[0].scene_type == "RELATION_ACTIVE", "active relation must be first"
    assert scenes[0].interest_level == "MEDIUM"
    assert any(s.scene_type == "COALITION_STRONG" for s in scenes)

    print("OK pf_battlefield_radar V0.2")
