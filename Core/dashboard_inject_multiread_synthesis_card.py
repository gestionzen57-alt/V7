...
START = "<!-- POWERFLOW_V734_MULTIREAD_SYNTHESIS_CARD_START -->"
END = "<!-- POWERFLOW_V734_MULTIREAD_SYNTHESIS_CARD_END -->"


def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--dashboard", default="dashboard_live.html")
    parser.add_argument("--card", default="dashboard_multiread_synthesis_card_patch.html")
    ...
    backup = dashboard.with_name(dashboard.name + ".bak_multiread_synthesis_" + datetime.now().strftime("%Y%m%d_%H%M%S"))
