...
$TaskRun = "cmd.exe /c cd /d $CorePath && python scheduler_powerflow_ontology_wrapper.py$SymbolsArg >> $LogPath 2>&1"
...
if ($Action -eq "enable" -or $Action -eq "update") {
    ...
    Write-Host "PASS enable/update: $TaskName" -ForegroundColor Green
