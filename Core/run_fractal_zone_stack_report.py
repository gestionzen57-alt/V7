"""
PowerFlow V6 - run_fractal_zone_stack_report.py
CLI Windows/Linux pour detecter les empilements fractals de zones.

Exemple Windows:
    python run_fractal_zone_stack_report.py --db powerflow.db --symbol GBPUSD --timeframes 1,5,15,30,60 --top 20
...
from pf_fractal_zone_stack import build_fractal_stack_report
...
def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="PowerFlow fractal zone stack report - scene multi-TF")
    ...
    parser.add_argument("--top", type=int, default=20, help="Nombre de stacks a afficher")
    parser.add_argument("--gap-minutes", type=float, default=45.0, help="Gap temporel max pour relier deux sequences")
    parser.add_argument("--min-score", type=float, default=5.0, help="Score minimum d'une sequence locale")
    ...
    try:
        report = build_fractal_stack_report(
            db_path=str(db_path),
            symbol=args.symbol,
            ...
            min_sequence_score=args.min_score,
