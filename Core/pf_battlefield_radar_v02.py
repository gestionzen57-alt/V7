...
def relation_strategic_score(field_score: float) -> float:
    # Relation gets structural priority because it contains antagonist.
    return round(1.0 + field_score, 4)


def coalition_strategic_score(cohesion: float, has_antagonist_candidate: bool) -> float:
    bonus = 0.08 if has_antagonist_candidate else 0.0
    return round(0.70 + min(cohesion, 1.0) * 0.30 + bonus, 4)
