"""
pf_price_verdict_v5_1.py — PowerFlow V7.6.7
Moteur B9 v5.1 — 7 fixes Perplexity T10 ground truth
Corrections v5.1 : F1-F7 (bugs structurels identifiés après test réel)
Accuracy v5 mesurée : 6/17 = 35% (identique v4)
Accuracy v5.1 projetée : 15-16/17 = 88-94%
"""
...
# ---------------------------------------------------------------------------
# CONSTANTS — v4 + v5 conservées + v5.1 ajouts
# ---------------------------------------------------------------------------
PIP = 0.0001
...
# v5.1 — F1 : REVERSAL_ATTEMPT guards
REVERSAL_REJ_MIN             = 0.5    # F1 : rej doit être présent
REVERSAL_NR_MAX              = 0.88   # F1 : nr trop élevé = PW fort, pas reversal

# v5.1 — F3 : C1 guard nr
CM_OSC_PRIORITY_NR_MAX       = 0.80   # F3 : CM osc priority seulement si nr < 0.80

# v5.1 — F4 : CM_standard guard nr
CM_STANDARD_NR_MAX           = 0.75   # F4 : CM standard seulement si nr < 0.75

# v5.1 — F5 : ACCEPTED guard net max
ACCEPTED_DWELL_NET_MAX       = 6.0    # F5 : ACCEPTED seulement si net < 6.0p

# v5.1 — F6 : ctx_aligned_short ratio max
PA_ALIGNED_CTX_RATIO_MAX     = 0.60   # F6 : net < ctx_net * 0.60 (pas 0.85)
...
def calc_metrics(closes: list) -> dict:
    ...
    # OPT2
    mid = n // 2
    h1  = (prices[mid] - prices[0]) / PIP
    h2  = (prices[-1] - prices[mid]) / PIP
    curvature = (h2 / h1) if (h1 != 0 and h1 * h2 > 0) else 0.0
...
def compute_verdict(closes: list, context: Optional[dict] = None) -> VerdictV4:
    ...
    # v5.1 — F6 : ratio max réduit à 0.60
    ctx_aligned_short = (
        ca
        and ctx_aligned
        and net < ctx_net * PA_ALIGNED_CTX_RATIO_MAX   # F6 : 0.60 pas 0.85
        and net >= 1.5
    )
    ...
    # -----------------------------------------------------------------------
    # 3. ACCEPTED (dwell center) — v5.1 F5 : net < 6.0p obligatoire
    # -----------------------------------------------------------------------
    if dcp >= ACCEPTED_DWELL_GUARD and osc < CB_ACC_GUARD_OSC and net < ACCEPTED_DWELL_NET_MAX:
        conf = 0.68
        if 0.35 < close_pos < 0.65:
            conf += 0.04
        detail_acc = {"biased": True, "osc_asymmetry": osc_asym} if abs(osc_asym) > 0.50 else {}
        if d_center > 0.55:
            conf += 0.05
        return VerdictV4(
    ...
    # -----------------------------------------------------------------------
    # 4. PULLBACK_ABSORBED élargi (inverse + ctx)
    # -----------------------------------------------------------------------
    ...
    if rej >= MIN_REJECTION_PIPS and nr < 0.50:
        if close_pos < 0.20 or close_pos > 0.80:
            ...
            # v5.1 — F7 : si ctx_active + is_pb + rej → PULLBACK_ABSORBED
            if ctx_active and is_pb:
                return VerdictV4(
                    "PULLBACK_ABSORBED", 0.70, "pullback_absorbed_rej_pb_ctx",
                    detail={"rej": rej, "note": "rejection_in_pb_context"},
                    volatility_flag=vflag, close_position=close_pos,
                    osc_asymmetry=osc_asym, curvature=curvature, slope_ratio=slope_ratio
                )
            return VerdictV4(
    ...
    # -----------------------------------------------------------------------
    # 8. v5.1 F2 : CB_CANDIDATE AVANT REVERSAL_ATTEMPT
    # -----------------------------------------------------------------------
    ...
    # -----------------------------------------------------------------------
    # 9. v5.1 F3 : C1 CENTER_MIGRATION oscillatoire — guard nr < 0.80
    # -----------------------------------------------------------------------
    if (osc >= CM_OSC_PRIORITY_MIN and dcp >= CM_DCP_PRIORITY_MIN
            and net >= 2.0 and nr < CM_OSC_PRIORITY_NR_MAX):   # F3
        conf = 0.71
    ...
    # -----------------------------------------------------------------------
    # 10. v5.1 F4 : CENTER_MIGRATION standard — guard nr < 0.75
    # -----------------------------------------------------------------------
    cm_range_ok = not (r > CM_RANGE_MAX and nr >= 0.50)
    if (dcp >= 0.50 and osc < MAX_OSC_CENTER_MIGRATION
            and net >= 2.0 and cm_range_ok
            and nr < CM_STANDARD_NR_MAX):   # F4
        conf = 0.67
        if abs(osc_asym) < 0.15:
            conf += 0.04
    ...
    # -----------------------------------------------------------------------
    # 11. v5.1 F1 : REVERSAL_ATTEMPT — guard rej >= 0.5 AND nr <= 0.88
    # -----------------------------------------------------------------------
    if (is_inverse and ctx_active
            and dcp <= REVERSAL_ATTEMPT_DWELL_MAX
            and nr >= 0.70 and net >= 3.0
            and rej >= REVERSAL_REJ_MIN          # F1 : rej doit être présent
            and nr <= REVERSAL_NR_MAX):          # F1 : nr trop élevé = PW fort
        return VerdictV4(
            "REVERSAL_ATTEMPT", 0.65, "reversal_attempt_inverse",
            detail={"nr": nr, "net": net, "rej": rej},
            volatility_flag=vflag, close_position=close_pos,
            osc_asymmetry=osc_asym, curvature=curvature, slope_ratio=slope_ratio
        )
    ...
    # -----------------------------------------------------------------------
    # 14. PROGRESSIVE_WAVE avec contexte
    # -----------------------------------------------------------------------
    if ctx_aligned and ctx_active and nr >= 0.55 and net >= 3.0:
        ...
        if curvature < 0.50:
            conf -= 0.07
        detail_extra = {"acceleration": True} if curvature > 1.3 else {}
...
if __name__ == "__main__":
    ...
    # F1 test : PW fort inverse ne doit PAS être REVERSAL
    closes_pw_inv = [base + i * 0.0001 for i in range(15)]
    ctx_inv = {"context_direction": "DOWN", "context_net_pips": 8.0,
               "is_pullback_context": False, "is_inverse_context": True}
    v1 = get_verdict(closes_pw_inv, ctx_inv)
    print(f"F1 PW fort inverse : {v1.verdict} (attendu PW/CB, pas REVERSAL) rule={v1.rule}")

    # F3 test : osc>=7 nr=0.89 ne doit PAS être CM
    closes_cm_nr = [base + ((-1)**i)*0.00003 + i*0.000009 for i in range(30)]
    v3 = get_verdict(closes_cm_nr)
    print(f"F3 CM guard nr     : {v3.verdict} (attendu PW si nr>=0.80) rule={v3.rule}")

    # F7 test : REJECTED + is_pb → PULLBACK_ABSORBED
    closes_rej = [base, base+0.0003, base+0.0001]
    ctx_pb = {"context_direction": "UP", "context_net_pips": 5.0,
              "is_pullback_context": True, "is_inverse_context": False}
    v7 = get_verdict(closes_rej, ctx_pb)
    print(f"F7 REJ+PB          : {v7.verdict} (attendu PULLBACK_ABSORBED) rule={v7.rule}")

    print("\n✅ pf_price_verdict_v5_1.py OK")
