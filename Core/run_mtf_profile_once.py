...
def main() -> int:
    ...
    profile = base / "mtf_profile.json"
    mem = base / "mtf_session_memory.json"
    md = base / "mtf_session_memory.md"

    cmd = [sys.executable, "pf_time_profile_window.py", "--db", args.db, "--symbol", args.symbol, "--profile", "MTF", "--broker-offset-hours", str(getattr(args, "broker_offset_hours", 1.0)),
        "--output", str(profile)]
    if args.pretty:
        cmd.append("--pretty")
    run(cmd)
    ...
    print(f"MTF_PROFILE_ONCE_OK | symbol={args.symbol} | profile={profile} | memory={mem}")
    return 0
