#!/usr/bin/env python3
"""
Inject FLOW_ONTOLOGY dashboard card into dashboard_live.html.
...
MARKER = 'data-brick="FLOW_ONTOLOGY_V0"'
...
def inject_card(dashboard: Path, card: Path) -> dict:
    ...
    backup = dashboard.with_suffix(dashboard.suffix + f".bak_flow_ontology_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Inject Flow Ontology dashboard card.")
    parser.add_argument("--dashboard", default="dashboard_live.html")
    parser.add_argument("--card", default="dashboard_flow_ontology_card_patch.html")
