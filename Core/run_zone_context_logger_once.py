...
Mission:
    ...
    with basket mode by default, run pf_zone_dynamics, and persist the latest diagnosis for each
...
This script is intentionally read-light/write-small:
    ...
    - default mode is basket, compatible with pf_personalities z_basket doctrine
"""
...
CURRENCIES = ["USD", "GBP", "EUR", "JPY", "CAD", "CHF", "AUD"]
...
def basket_pressure_for_row(row: sqlite3.Row, currency: str) -> Optional[float]:
    """Currency pressure against the available force basket.

    This mirrors the pf_personalities z_basket doctrine:
        mean(force_currency - force_other)

    It also gives USD its own readable dynamic zone:
        USD vs basket, not USD vs USD.
    """
    own = get_force_value(row, currency)
    if own is None:
        return None

    spreads: List[float] = []
    for other in ALL_RANK_CURRENCIES:
        if other == currency:
            continue
        other_force = get_force_value(row, other)
        if other_force is not None:
            spreads.append(own - other_force)
...
def usd_spread_for_row(row: sqlite3.Row, currency: str) -> Optional[float]:
    """Legacy direct spread versus USD.

    Kept as an explicit mode for comparison, but not the V6.1 default.
    """
    cur_force = get_force_value(row, currency)
    usd_force = get_force_value(row, "USD")
    if cur_force is None or usd_force is None:
        return None
    return cur_force - usd_force
...
def analyze_latest_for_timeframe(
    ...
    *,
    ...
    for currency in currencies:
        ...
        for row in rows:
            if mode == "usd":
                values.append(usd_spread_for_row(row, currency))
            else:
                values.append(basket_pressure_for_row(row, currency))

        z_series = rolling_z(values, prof.lookback)
...
def main() -> int:
    ...
    parser.add_argument("--mode", choices=["basket", "usd"], default="basket", help="Z-source mode: basket is V6.1 default; usd keeps legacy direct spread")
    ...
    print(f"OK logged/kept {len(all_ids)} zone diagnostics into {args.db} [mode={args.mode}]")
    if args.summary:
...
# V0.1.1 patch:
# - Adds USD to logged currencies.
# - Uses basket pressure by default: mean(force_currency - force_other).
# - Keeps --mode usd for legacy currency-vs-USD comparison.
# - Aligns the runner with pf_personalities z_basket doctrine.
