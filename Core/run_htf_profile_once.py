...
def main() -> int:
    ...
    profile = base / "htf_profile.json"
    mem = base / "htf_session_memory.json"
    md = base / "htf_session_memory.md"

    cmd = [sys.executable, "pf_time_profile_window.py", "--db", args.db, "--symbol", args.symbol, "--profile", "HTF", "--broker-offset-hours", str(getattr(args, "broker_offset_hours", 1.0)),
        "--output", str(profile)]
    if args.pretty:
        cmd.append("--pretty")
    run(cmd)
    ...
    print(f"HTF_PROFILE_ONCE_OK | symbol={args.symbol} | profile={profile} | memory={mem}")
    return 0
