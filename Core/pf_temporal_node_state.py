...
# --- energy_release_alignment imports (V0.8.2) — defensive ---
try:
    from pf_currency_energy_probe import build_currency_energy_state as _build_currency_energy_state
    _CURRENCY_ENERGY_AVAILABLE = True
except ImportError:
    _CURRENCY_ENERGY_AVAILABLE = False
# --- end energy_release_alignment imports ---
...
def _build_kinematics_state(
    ...
    # -----------------------------------------------------------------------
    # RELEASE_CANDIDATE / RELEASE_STATE — V0.8.1
    ...
    has_m1_energy_fade = "M1_ENERGY_FADE_LAST3" in accel_tags
    has_m1_acceleration = any("M1_GBP_ACCEL" in t or "M1_USD_ACCEL" in t for t in accel_tags)

    if has_m1_energy_fade:
        rc_ok.append("m1_energy_fade_relay")
    elif has_m1_acceleration:
    ...
    # Objectif : distinguer une release dans le sens du relais d'une contre-release.
    m5_gbp_angle = _angle(m5_cur, "GBP")
    m5_usd_angle = _angle(m5_cur, "USD")
    m5_supports_detachment = False
    if detachment_found and m5_gbp_angle is not None and m5_usd_angle is not None:
        if detachment_currency == "GBP" and detachment_direction == "UP":
            m5_supports_detachment = m5_gbp_angle > 5 and m5_usd_angle < 5
        elif detachment_currency == "GBP" and detachment_direction == "DOWN":
            m5_supports_detachment = m5_gbp_angle < -5 and m5_usd_angle > -5
        elif detachment_currency == "USD" and detachment_direction == "UP":
            m5_supports_detachment = m5_usd_angle > 5 and m5_gbp_angle < 5
        elif detachment_currency == "USD" and detachment_direction == "DOWN":
            m5_supports_detachment = m5_usd_angle < -5 and m5_gbp_angle > -5
    ...
    fade_confirmed = bool(force_hold_with_acceleration_fade.get("detected"))

    # Typage V0.8.1.
    # RELEASE_CONFIRMED est volontairement strict.
    # COUNTER_RELEASE_ATTEMPT reste une perception exploitable mais non confirmée.
    if not first_detachment.get("detected"):
        release_state = "RELEASE_REJECTED"
        release_confidence = "LOW"
        rc_label = "RELEASE_REJECTED_NO_DETACHMENT"
    elif first_detachment.get("detected") and not price_coherent:
        release_state = "FAKE_RELEASE"
        release_confidence = "LOW"
        rc_label = "FAKE_RELEASE_PRICE_DIVERGENCE"
    elif relay_label in ("MISSING", "BLOCKED", "STALE"):
        release_state = "RELEASE_REJECTED"
        release_confidence = "LOW"
        rc_label = f"RELEASE_REJECTED_RELAY_{relay_label or 'UNKNOWN'}"
    elif first_detachment.get("detected") and price_coherent and not m5_supports_detachment:
        release_state = "COUNTER_RELEASE_ATTEMPT"
        release_confidence = "MEDIUM" if (has_m1_energy_fade or has_m1_acceleration) else "LOW"
        rc_dir = detachment_direction if detachment_found else "UNKNOWN"
        rc_label = f"COUNTER_RELEASE_ATTEMPT_{rc_dir}"
    elif first_detachment.get("detected") and price_coherent and m5_supports_detachment and fade_confirmed and relay_label == "CLEAN":
        release_state = "RELEASE_CONFIRMED"
        release_confidence = "STRONG"
        rc_dir = detachment_direction if detachment_found else "UNKNOWN"
        rc_label = f"RELEASE_CONFIRMED_{rc_dir}"
    elif first_detachment.get("detected") and price_coherent and m5_supports_detachment:
        release_state = "RELEASE_CANDIDATE"
        release_confidence = "MEDIUM" if relay_label == "CLEAN" else "LOW"
        rc_dir = detachment_direction if detachment_found else "UNKNOWN"
        rc_label = f"RELEASE_CANDIDATE_{rc_dir}"
    else:
        release_state = "RELEASE_ATTEMPT"
        release_confidence = "LOW"
        rc_dir = detachment_direction if detachment_found else "UNKNOWN"
        rc_label = f"RELEASE_ATTEMPT_{rc_dir}"

    release_confirmed = release_state == "RELEASE_CONFIRMED"
    release_watch = release_state in {
        "RELEASE_ATTEMPT",
        "RELEASE_CANDIDATE",
    ...
    # Compatibilité aval :
    # - release reste conservé, mais ne vaut true que pour CANDIDATE/CONFIRMED.
    # - COUNTER_RELEASE_ATTEMPT n'est donc plus assimilé à release confirmée.
    release_candidate = {
        "release": release_state in {"RELEASE_CANDIDATE", "RELEASE_CONFIRMED"},
        ...
        "m5_supports_detachment": m5_supports_detachment,
        "m5_gbp_angle": m5_gbp_angle,
        "m5_usd_angle": m5_usd_angle,
        "reasons_ok": rc_ok,
        "reasons_nok": rc_nok,
        "flags": rc_flags,
    }
    ...
    if release_state in {"RELEASE_ATTEMPT", "RELEASE_CANDIDATE", "RELEASE_CONFIRMED", "COUNTER_RELEASE_ATTEMPT"}:
        lab_signatures.append(rc_label)
    if release_state == "RELEASE_CONFIRMED":
        lab_signatures.append("RELEASE_CONFIRMED")
    elif release_state == "COUNTER_RELEASE_ATTEMPT":
        lab_signatures.append("COUNTER_RELEASE_ATTEMPT_NOT_CONFIRMED")
    elif release_state == "FAKE_RELEASE":
        lab_signatures.append("FAKE_RELEASE")
    elif release_state == "RELEASE_REJECTED":
        lab_signatures.append("RELEASE_REJECTED")
...
# ---------------------------------------------------------------------------
# V0.8.2 — energy_release_alignment builder
# ---------------------------------------------------------------------------

def _build_energy_release_alignment(
    db_path: str,
    symbol: str,
    kinematics_state: dict,
    capture_quality: dict,
    bars: int = 50,
) -> dict:
    """
    Produit state["energy_release_alignment"] — bloc additif non destructif.

    Règle centrale :
    - Currency Energy qualifie release_state.
    - Currency Energy ne crée pas de signal.
    ...
    def _unavailable(reason: str) -> dict:
        return {
            "status": "UNAVAILABLE",
            "reason": reason,
            "state": "ENERGY_ALIGNMENT_UNAVAILABLE",
            "secondary_state": None,
            "field_quality": "UNKNOWN",
            "release_state": None,
            "first_detachment": None,
    ...
    if not _CURRENCY_ENERGY_AVAILABLE:
        return _unavailable("module_not_found:pf_currency_energy_probe")

    release_candidate = (kinematics_state or {}).get("release_candidate", {})
    first_detachment = (kinematics_state or {}).get("first_detachment", {})

    release_state = release_candidate.get("release_state", "UNKNOWN")
    first_det_label = first_detachment.get("label", "UNKNOWN")
    first_det_detected = bool(first_detachment.get("detected", False))

    def _call_energy(tf: int, htf_tfs: list[int]) -> dict:
        """
        Appel défensif de build_currency_energy_state.
        Plusieurs signatures sont acceptées pour éviter un couplage fragile.
        """
        dbp = _Path(db_path)
        attempts = [
            lambda: _build_currency_energy_state(
                db_path=dbp,
                symbol=symbol.upper(),
                timeframe=tf,
                bars=bars,
        ...
        for attempt in attempts:
            try:
                ...
                last_error = exc
                continue
        raise last_error if last_error is not None else RuntimeError("currency_energy_call_failed")

    def _energy_label_strength(label: str) -> int:
        order = {
            "ENERGY_WEAK": 0,
            "ENERGY_LOW": 1,
            "ENERGY_MEDIUM": 2,
    ...
    def _currency_snapshot(energy_state: dict, currency: str) -> dict:
        c = energy_state.get("currencies", {}).get(currency, {}) or {}
        raw = c.get("raw_signed", {}) or {}
        return {
            "score": c.get("energy_score"),
            "label": c.get("energy_label"),
            "absorption": c.get("absorption_escape_state"),
            "zone_state": raw.get("zone_state"),
            "zone_level": raw.get("zone_level"),
            "z_extreme_dir": raw.get("z_extreme_dir"),
            "behavioral_zscore": raw.get("behavioral_zscore"),
            "speed_per_min": raw.get("speed_per_min"),
            "angle_deg": raw.get("angle_deg"),
            "acceleration_raw": raw.get("acceleration_raw"),
            "role": raw.get("role"),
        }

    def _is_weak_or_neutral(snap: dict) -> bool:
        return snap.get("label") == "ENERGY_WEAK" or snap.get("absorption") == "NEUTRAL"

    def _has_micro_tension(snap: dict) -> bool:
        return (
            snap.get("label") in {"ENERGY_MEDIUM", "ENERGY_HIGH"}
            or snap.get("absorption") in {"EARLY_EXTREME", "PRE_EXTREME", "ACCUMULATING", "LEAKING", "RUPTURE"}
        )

    def _pair_vote(tf_label: str, energy_state: dict) -> tuple[str, list[str], dict]:
        gbp = _currency_snapshot(energy_state, "GBP")
        usd = _currency_snapshot(energy_state, "USD")
        top = energy_state.get("top_energy", {}) or {}
        summary = energy_state.get("energy_field_summary", "")

        reasons: list[str] = []
        gbp_weak = _is_weak_or_neutral(gbp)
        usd_weak = _is_weak_or_neutral(usd)
        gbp_tension = _has_micro_tension(gbp)
        usd_tension = _has_micro_tension(usd)

        gbp_strength = _energy_label_strength(str(gbp.get("label")))
        usd_strength = _energy_label_strength(str(usd.get("label")))

        if gbp_weak and usd_weak:
            vote = "GBP_USD_WEAK_NEUTRAL"
            reasons.append(f"{tf_label}:GBP/USD weak-or-neutral")
        elif gbp_tension and usd_weak:
            vote = "GBP_TENSION_WITHOUT_USD_PAIR_RELEASE"
            reasons.append(f"{tf_label}:GBP energy present while USD weak/neutral")
        elif usd_tension and gbp_weak:
            vote = "USD_TENSION_WITHOUT_GBP_PAIR_RELEASE"
            reasons.append(f"{tf_label}:USD energy present while GBP weak/neutral")
        elif abs(gbp_strength - usd_strength) <= 1 and gbp_strength >= 2 and usd_strength >= 2:
            vote = "GBP_USD_BOTH_ACTIVE_PAIR_FIELD"
            reasons.append(f"{tf_label}:GBP/USD both active")
        else:
            vote = "GBP_USD_MIXED_ENERGY_FIELD"
            reasons.append(f"{tf_label}:GBP/USD mixed energy")
        ...
        return vote, reasons, snapshot

    energy_states: dict[str, dict] = {}
    errors: list[str] = []
    for tf, label, htf_tfs in [
        (1, "M1", [15, 30, 60]),
        (5, "M5", [15, 30, 60]),
        (15, "M15", [30, 60]),
    ]:
        try:
            energy_states[label] = _call_energy(tf, htf_tfs)
        except Exception as exc:
            errors.append(f"{label}:{type(exc).__name__}:{exc}")

    if errors:
        return _unavailable("energy_probe_error:" + "|".join(errors))

    tf_votes: dict[str, str] = {}
    vote_reasons: list[str] = []
    energy_snapshots: dict[str, dict] = {}

    for label in ("M1", "M5", "M15"):
        vote, reasons, snapshot = _pair_vote(label, energy_states.get(label, {}))
        tf_votes[label] = vote
        vote_reasons.extend(reasons)
        energy_snapshots[label] = snapshot
    ...
    if (
        release_state == "RELEASE_REJECTED"
        and (not first_det_detected or first_det_label == "NO_DETACHMENT")
        and m5_vote == "GBP_USD_WEAK_NEUTRAL"
        and m15_vote == "GBP_USD_WEAK_NEUTRAL"
    ):
        alignment_state = "ENERGY_SUPPORTS_REJECTION"
        reasons.append("release_rejected_no_first_detachment")
        reasons.append("tf5_tf15_gbp_usd_weak_neutral")
    elif release_state in {"RELEASE_CANDIDATE", "RELEASE_CONFIRMED"} and m5_vote == "GBP_USD_BOTH_ACTIVE_PAIR_FIELD":
        alignment_state = "ENERGY_CONFIRMS_RELEASE_STATE"
        reasons.append("release_state_has_pair_energy_support")
    elif release_state == "RELEASE_REJECTED" and m5_vote == "GBP_USD_BOTH_ACTIVE_PAIR_FIELD":
        alignment_state = "ENERGY_CONTRADICTS_RELEASE_STATE"
        reasons.append("release_rejected_but_m5_pair_energy_active")
    elif release_state == "COUNTER_RELEASE_ATTEMPT":
        alignment_state = "ENERGY_SUPPORTS_COUNTER_RELEASE" if m1_vote != "GBP_USD_WEAK_NEUTRAL" else "ENERGY_NEUTRAL_OR_TOO_THIN"
        reasons.append("counter_release_attempt_energy_qualified")
    else:
        alignment_state = "ENERGY_NEUTRAL_OR_TOO_THIN"
        reasons.append("no_clear_energy_release_alignment")

    if (
        m1_vote in {"GBP_TENSION_WITHOUT_USD_PAIR_RELEASE", "USD_TENSION_WITHOUT_GBP_PAIR_RELEASE"}
        and m5_vote == "GBP_USD_WEAK_NEUTRAL"
    ):
        secondary_state = "ENERGY_SUPPORTS_ABSORPTION_NOT_RELEASE"
        reasons.append("micro_tension_without_pair_release")
    elif m1_vote == "GBP_TENSION_WITHOUT_USD_PAIR_RELEASE":
        secondary_state = "ENERGY_SUPPORTS_MICRO_TENSION"
        reasons.append("m1_gbp_tension_usd_weak")

    if secondary_state in {"ENERGY_SUPPORTS_ABSORPTION_NOT_RELEASE", "ENERGY_SUPPORTS_MICRO_TENSION"}:
        field_quality = "MIXED_BIPOLAR_FIELD"
    elif alignment_state == "ENERGY_SUPPORTS_REJECTION":
        field_quality = "PAIR_ENERGY_NOT_CONFIRMED"
    elif alignment_state == "ENERGY_CONFIRMS_RELEASE_STATE":
        field_quality = "PAIR_ENERGY_CONFIRMED"
    elif alignment_state == "ENERGY_CONTRADICTS_RELEASE_STATE":
        field_quality = "ENERGY_RELEASE_CONFLICT"
    else:
    ...
    return {
        ...
        "release_state": release_state,
        "release_label": release_candidate.get("label"),
        "first_detachment": first_det_label,
        "first_detachment_detected": first_det_detected,
        "relay_quality": (capture_quality or {}).get("relay_quality"),
        "relay_sample_state": (capture_quality or {}).get("relay_sample_state"),
        "tf_votes": tf_votes,
        "relation": "energy qualifies release_state; energy does not create signal",
        "reasons": reasons,
...
def build_temporal_node_state(
    ...
    # --- V0.8.2 energy_release_alignment (non destructif) ---
    try:
        state["energy_release_alignment"] = _build_energy_release_alignment(
            db_path=db_path,
            symbol=symbol,
            kinematics_state=state.get("kinematics_state", {}),
            capture_quality=state.get("capture_quality", {}),
            bars=50,
        )
    except Exception as _energy_exc:
        state["energy_release_alignment"] = {
            "status": "UNAVAILABLE",
            "reason": f"outer_exception:{type(_energy_exc).__name__}:{_energy_exc}",
            "state": "ENERGY_ALIGNMENT_UNAVAILABLE",
            "secondary_state": None,
            "field_quality": "UNKNOWN",
            "relation": "energy qualifies release_state; energy does not create signal",
