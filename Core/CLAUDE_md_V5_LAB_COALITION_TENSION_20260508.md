# CLAUDE.md — PowerFlow V6 Complete Context V5
**Date**: 2026-05-08  
**Status**: PRODUCTION — HTF + Orchestral + Lab V3 (11 couches)  
**Project**: PowerFlow — Market Flow Understanding Engine (Python)  
**Size**: ~35 KB densified | **Token Save**: 900+ per session
...
pf_orchestral_gravity_v02.py V0.2 — leader/follower/croisements/coalitions + zone_dynamics
...
### Orchestral Cockpit Integration V0.1.4 (VALIDATED M2 ✅)

```
File: cockpit_agentic_state_v01_orchestral.py  V0.1.4
Adds: bloc orchestral_gravity dans cockpit state JSON
Args: --orchestral-tfs "1,5,15,30"  --orchestral-avg-bars 3

Output bloc:
  state              ORCHESTRAL_ACTIVE / ORCHESTRAL_ALL_TF_FAILED / ORCHESTRAL_BRIDGE_ERROR
  timeframes         OrchestraState par TF
  latest_tf          TF numériquement le plus élevé disponible
  latest_state       OrchestraState du latest_tf
  compression_detected  bool auto-détecté depuis patterns
  leader_currency    Devise leader du latest_state
  patterns           Liste patterns nommés du latest_state
```

### Orchestral Loop Live (VALIDATED M1 ✅)

```
File: run_orchestral_loop.py
Purpose: boucle live compute + write orchestral state JSON à interval régulier

Features:
  ✅ Boucle infinie configurable (default 60s)
  ✅ Fenêtre glissante auto (lookback depuis NOW)
  ✅ Write JSON overwrite (dashboard) ou timestampé (historique)
  ✅ Graceful shutdown SIGINT/SIGTERM (sleep interruptible < 1s)
  ✅ Per-TF error isolation (1 TF fail n'affecte pas les autres)
  ✅ --once mode pour test rapide

Args:
  --db, --symbol, --tfs, --interval, --lookback, --avg-bars
  --output, --no-overwrite, --pretty, --once, --log-level

NOT YET:
  ❌ lab.py queries orchestrales (M3 — autre fil)
...
### Active Engine Chain — Orchestral Gravity (VALIDATED + LOOP LIVE ✅)
```
pf_force_inflection.py (V0.1, pliures contresens)
pf_force_extrema.py (V0.1, valleys/peaks asymétrie)
pf_orchestral_gravity_v02.py (V0.2, leader/follower/coalitions)
run_orchestral_analysis_once.py (runner ponctuel start/end manuel)
run_orchestral_loop.py (M1 ✅ — boucle live fenêtre glissante)
```

### Cockpit/Dashboard Chain (LIVE ✅)
```
cockpit_agentic_state_v01_orchestral.py (V0.1.4, M2 ✅ — avec bloc orchestral)
...
orchestral_live.json (M1 ✅ — output loop orchestral)
```
...
### Radar/Coalition Chain (LAB 🟢)
```
pf_coalitions.py               (coalition detection — LAB layer 10)
pf_coalition_relations.py      (battlefield relations — LAB layer 10)
pf_tension_signature.py        (micro/macro variance — LAB layer 11)
pf_personalities.py            (profils devises — coalition deps)
pf_battlefield_radar.py        (legacy — à migrer)
pf_battlefield_map.py          (legacy — à migrer)
pf_cockpit_field.py            (legacy — à migrer)
```
...
### Lab PowerFlow V3 — 11 Couches (LIVE 08/05 ✅)
```
pf_lab_engine.py               (moteur 11 couches)
lab_powerflow.py               (CLI runner)
pf_lab_coalitions.py           (layer 10 — coalitions)
pf_lab_tension.py              (layer 11 — tension signature)

Couches:
  1. kinematics        angle/speed/accel/first_detachment/clusters
  2. zones             NEUTRAL→PRE_EXTREME→ACCUMULATING→RUPTURE + cascade
  3. nodes             fractal nodes (TRIPLE_CROSS_CLUSTER, PRE_CROSS_COMPRESSION...)
  4. turning_points    naissances mouvement (BIRTH/CONFIRMED/WATCH)
  5. orchestra         leader/follower/compression orchestrale
  6. relational        gravity legacy
  7. fractal           cohérence LTF/HTF (PHASE_SYNC/OPPOSITION/PARTIAL)
  8. relational_gravity  gravity direct multi-TF (bypass P1.2)
  9. temporal_density    COMPRESSED/ACTIVE/HOLLOW/DEAD per devise
 10. coalition         blocs coalitions + battlefield windows ← NEW
 11. tension_signature  ELASTIC_LOADED/DIRECTIONAL_MOVE/DEAD_CURRENCY ← NEW

Queries: kinematics, zones, nodes, turning_points, orchestra, relational,
         fractal, relational_gravity, temporal_density,
         coalition, tension, full_v2 (1-9), full_v3 (1-11)
```

---

## 3. LEXIQUE COMPLET V4 — HTF + ORCHESTRAL + LOOP
...
### Orchestral Cockpit Block — États (NEW M2 07/05)
```
ORCHESTRAL_ACTIVE                  Au moins 1 TF valide dans cockpit block
ORCHESTRAL_NO_TIMEFRAMES           Aucun TF fourni pour analyse
ORCHESTRAL_BRIDGE_ERROR            Erreur globale bridge orchestral
ORCHESTRAL_TF_ERROR                Erreur compute TF individuel
ORCHESTRAL_ALL_TF_FAILED           Tous les TFs ont échoué
LATEST_TF (orchestral)             TF numériquement le plus élevé disponible
LATEST_STATE (orchestral)          OrchestraState du latest_tf
COMPRESSION_DETECTED               Bool auto depuis patterns (ORCHESTRAL_COMPRESSION présent)
LEADER_CURRENCY                    Devise leader du latest_state
ORCHESTRAL_AVG_BARS                Barres moyennées pour angle (default 3)
ORCHESTRAL_TIMEFRAMES (config)     Liste TFs configurés pour analyse cockpit
```

### Orchestral Loop Live — États (NEW M1 07/05)
```
ORCHESTRAL_LOOP                    Boucle live compute + write JSON interval régulier
ORCHESTRAL_LOOP_ERROR              Exception globale dans run_loop()
ORCHESTRAL_TF_NO_DATA              TF individuel sans données suffisantes (loop)
OVERWRITE_MODE                     1 fichier JSON réutilisé — dashboard-friendly
TIMESTAMPED_MODE                   Fichier horodaté par iteration — trace historique
LOOKBACK_WINDOW                    Fenêtre glissante depuis NOW (default 180 min)
ONCE_MODE                          Single run → stdout, test rapide (--once)
GRACEFUL_SHUTDOWN                  Arrêt propre SIGINT/SIGTERM (< 1s)
SLEEP_INTERRUPTIBLE                Sleep fragmenté 1s chunks — Ctrl+C réactif
valid_tfs                          Liste TFs ayant produit OrchestraState valide
latest_tf                          Max numérique de valid_tfs
window_start / window_end          Bornes ISO8601 de la fenêtre lookback courante
```
...
### Coalition — Lab Layer 10 (NEW 08/05)
```
CurrencyVector                     Vecteur devise: z_basket, slope, curvature, phase, context_tags
CurrencyCoalition                  Groupe devises respirant ensemble (z proche, slope alignée, curvature proche)
cohesion                           Score cohésion coalition [0..1], seuil 0.62
personality_compatibility_score    Compatibilité volatilité + rôle + tempo entre devises [0..1]

DEFAULT_MIN_ABS_Z = 1.20           Tension minimale pour entrer coalition (abs z_basket)
DEFAULT_MAX_Z_GAP = 0.55           Écart z_basket max entre membres coalition
DEFAULT_MAX_SLOPE_GAP = 0.18       Écart slope max entre membres
DEFAULT_MIN_COHESION = 0.62        Cohésion minimale coalition valide

LOW_ELASTIC_COALITION_RESPRING     Coalition basse (z<-2.0) rebondissant fort (RISING)
LOW_COALITION_RISING               Coalition basse montante standard (z<0, RISING, z>-2.0)
LOW_PRESSURE_COALITION_EXPANDING   Coalition basse s'enfonçant (z<0, FALLING)
HIGH_PRESSURE_COALITION_FOLDING    Coalition haute (z>+2.0) pliant fort (FALLING)
HIGH_COALITION_FALLING             Coalition haute descendante standard (z>0, FALLING, z<+2.0)
HIGH_PRESSURE_COALITION_EXPANDING  Coalition haute montant encore (z>0, RISING)

MICROFILM_SYNCHRONIZED_FIELD       Coalition M1 microfilm (courte durée)
INTERMEDIATE_SYNCHRONIZED_FIELD    Coalition M5/M15 intermédiaire
SCENARIO_SYNCHRONIZED_FIELD        Coalition H1+ scenario (durée longue)
SYNCHRONIZED_RESPRING              Plusieurs devises rebondissent ensemble depuis zone basse
SYNCHRONIZED_FOLDING               Plusieurs devises plient ensemble depuis zone haute
```

### Battlefield Relations — Lab Layer 10 (NEW 08/05)
```
CoalitionBattlefieldRelation       Relation opposition coalition vs antagoniste
LOW_BLOCK_RESPRING_AGAINST_HIGH_FOLDING    Bloc bas respring vs haut folding (rotation classique)
HIGH_BLOCK_FOLDING_AGAINST_LOW_RESPRING    Bloc haut folding vs bas respring (rotation inverse)
COALITION_VS_ANTAGONIST_OPPOSITION         Opposition générique (polarité + direction inverses)
POLARIZED_FIELD_WITH_WEAK_TIMING           Polarité présente mais timing faible
...
POLARITY_PRESENT_TIMING_WEAK       Opposition polarité forte, timing slope faible
STRUCTURE_BUILDING                 Construction structure (field_score < 0.58)
WEAK_FIELD_OPPOSITION              Opposition faible (opposition_score < 0.40)

ACTIVE_COALITION_ROTATION          Phase rotation coalition active
...
### Tension Signature — Lab Layer 11 (NEW 08/05)
```
TensionSignature                   Signature micro/macro variance devise
ELASTIC_LOADED                     Devise comprimée élastique en charge (score > 2.5)
                                   Micro-agitation haute, macro plat → attente release
DIRECTIONAL_MOVE                   Mouvement directionnel lent (score < 0.35)
                                   Macro dominante, micro faible → trend smooth
DEAD_CURRENCY                      Devise inactive ou pause (score 0.35-2.5 équilibré)
...
### Coalition Legacy (old)
```
COALITION                          Family of synchronized currencies (old term)
...
run_orchestral_analysis_once.py       ← ponctuel, start/end manuel
run_orchestral_loop.py (M1 ✅)        ← live loop, fenêtre glissante auto
    ↓ output/orchestral_live.json (overwrite) ou horodaté
    ↓
───────────────── BEHAVIORAL MAPPER ─────────────────
...
└─ OUTPUT: behavioral_alert_queue.json
    ↓
───────────────── COCKPIT AGGREGATION ─────────────────
cockpit_agentic_state_v01_orchestral.py (M2 ✅ V0.1.4)
├─ Aggregates all insights
├─ Includes: temporal_nodes, relational_gravity, orchestral_gravity ← NEW
...
from pf_orchestral_gravity_v02 import compute_orchestra_state, compute_orchestra_multi_tf
...
|------|---------|--------|-----------------|
| 08/05 | Lab V3 Coalition+Tension | ✅ LIVE | 11 couches opérationnelles |
| 07/05 | Orchestral Loop M1 | ✅ LIVE | run_orchestral_loop.py — boucle live 60s |
| 07/05 | Orchestral Cockpit M2 | ✅ LIVE | cockpit V0.1.4 — bloc orchestral_gravity |
...
### Run Orchestral Analysis (ponctuel)
```powershell
# Markdown
...
### Run Orchestral Loop Live (M1 ✅)
```powershell
# Default — LTF+M30, 60s interval, overwrite
...
python cockpit_agentic_state_v01_orchestral.py `
  ...
  --start "2026-05-07T07:00:00+00:00" `
  --end "2026-05-07T12:00:00+00:00" `
  --out output/cockpit_orchestral.json `
  --pretty
...
### Lab PowerFlow V3 — 11 Couches (NEW 08/05 ✅)
```powershell
# Coalition standalone
python lab_powerflow.py --query coalition `
  --db powerflow.db --symbol GBPUSD `
  --tfs "15,30,60" --coalition-bars 50 `
  --coalition-cohesion 0.62 --once --pretty

# Tension signature standalone
python lab_powerflow.py --query tension `
  --db powerflow.db --symbol GBPUSD `
  --tfs "15,30,60" --tension-bars 30 `
  --tension-window 5 --once --pretty

# Full V3 — toutes les 11 couches
python lab_powerflow.py --query full_v3 `
  --db powerflow.db --symbol GBPUSD `
  --horizons "MTF" --once --lookback 300 `
  --out output/lab_full_v3.json --pretty

# Full V3 — fenêtre fixe HTF
python lab_powerflow.py --query full_v3 `
  --db powerflow.db --symbol GBPUSD `
  --tfs "60,240,1440" `
  --start "2026-05-07T00:00:00" `
  --end "2026-05-07T23:59:59" `
  --coalition-bars 80 --tension-bars 50 `
  --out output/lab_htf.json --pretty

# Kinematics seule (layer 1)
...
### Orchestral Remaining (PARTIAL 🔵)
```
DONE:    run_orchestral_loop.py (M1 ✅)
DONE:    cockpit_agentic_state_v01_orchestral.py V0.1.4 (M2 ✅)
NOT YET: lab.py queries orchestrales (M3 — autre fil)
NOT YET: H4 support (manque données avg_bars)
NOT YET: dashboard_live.html display orchestral block
PRIORITY: P4
```
...
"Run orchestral loop test on powerflow.db"
```
...
### Structure
```
Claude       → Code / consolidation / patches / CLAUDE.md updates
...
🚫 Never start M3 lab.py in this thread (open new thread)
```
...
Lab V3 adds: coalition detection + tension signature + 11-layer queries
```
...
Before any new work:
- [ ] Read this CLAUDE.md V5
- [ ] Check CURRENT_STATE + CHECKPOINT_LATEST
- [ ] Check BLOCKERS section (P1.2 critical)
...
✅ Orchestral live: qui mène maintenant? compression détectée?
✅ Coalition detection: blocs devises synchronisés + battlefield windows
✅ Tension signature: ELASTIC_LOADED vs DIRECTIONAL_MOVE per devise
✅ Lab 11 layers: query multi-couches pour analyse complète
...
ELASTIC_LOADED_BEFORE_RELEASE (tension signature)
SECOND_LEG_WATCH
```
...
Orchestral Loop (M1) feeds live JSON every 60s.
Orchestral Cockpit (M2) exposes orchestral block in cockpit state.

Lab PowerFlow V3 (08/05) exposes 11 analytical layers:
  1. kinematics — angle/speed/accel/first_detachment
  2. zones — ACCUMULATING/LEAKING/RUPTURE cascade
  3. nodes — fractal nodes multi-horizon
  4. turning_points — BIRTH/CONFIRMED/WATCH
  5. orchestra — leader/follower/compression
  6. relational (legacy) — gravity brut
  7. fractal — LTF/HTF coherence PHASE_SYNC/OPPOSITION
  8. relational_gravity — gravity direct (bypass P1.2)
  9. temporal_density — COMPRESSED/ACTIVE/HOLLOW/DEAD
 10. coalition — blocs devises + battlefield windows
 11. tension_signature — ELASTIC_LOADED/DIRECTIONAL_MOVE

BLOCKER: P1.2 Bridge Guard must fix MIXED field clarity.
LIVE: Lab V3 coalition+tension opérationnel (08/05).
NEXT: Dashboard display coalitions → HTF_CONTEXT_STACK spec.
LATER: Telegram (much later).
...
Updated 2026-05-08.
HTF corrected. Orchestral live. Lab V3 11 couches opérationnel.
Coalition+Battlefield+Tension signature intégrés.
Relational P1.2 blocker documented. Multi-IA ready.
