...
def _build_orchestral_gravity(
    ...
    try:
        ...
        # Filtrer les TFs valides (non-error)
        valid_tfs = [int(k) for k in tf_states.keys() if tf_states[k].get("state") != "ORCHESTRAL_TF_ERROR"]
        ...
        if not valid_tfs:
            return {
                ...
                "latest_state": None,
                "compression_detected": False,
                "notes": ["All timeframes returned ORCHESTRAL_TF_ERROR"],
            }
        
        latest_tf = max(valid_tfs)
