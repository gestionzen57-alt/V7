...
V0.2:
    - active relations are prioritized above isolated coalitions
    - repeated scene families are deduped by default
    - output shows strategic_score
"""
...
def scene_sort_key(scene: Any) -> tuple:
    ...
    return (type_rank, scene.strategic_score, rank)


def dedupe_global_scenes(scenes: List[Any]) -> List[Any]:
    by_key = {}
    for scene in scenes:
        key = (
            scene.timeframe,
        ...
        if current is None or scene_sort_key(scene) > scene_sort_key(current):
            by_key[key] = scene
    out = list(by_key.values())
    out.sort(key=scene_sort_key, reverse=True)
    return out
    ...
    *,
    ...
    for tf in timeframes:
        if scan > 0:
            ...
            scenes = build_battlefield_scenes_from_scan_payload(payload, max_scenes=max_scenes, dedupe=dedupe)
        else:
            payload = run_latest(
                db,
                timeframe=tf,
            ...
            scenes = build_battlefield_scenes_from_latest_payload(payload, max_scenes=max_scenes, dedupe=dedupe)
    ...
    if dedupe:
        all_scenes = dedupe_global_scenes(all_scenes)
    else:
...
def print_payload(payload: Dict[str, Any]) -> None:
    print("PowerFlow Battlefield Radar — V0.2")
    print("=" * 72)
    ...
    for idx, scene in enumerate(payload["scenes"], start=1):
        print(
            ...
            f"{scene['scene_type']} | {scene['battle_state']} | "
            f"strategic={scene['strategic_score']:.2f} | {scene['cockpit_sentence']}"
        )
